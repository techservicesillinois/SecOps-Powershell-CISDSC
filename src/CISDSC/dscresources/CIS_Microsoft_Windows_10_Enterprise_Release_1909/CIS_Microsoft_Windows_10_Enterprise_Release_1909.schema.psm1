Configuration CIS_Microsoft_Windows_10_Enterprise_Release_1909
{
    param
    (
        [string[]]$ExcludeList = @(),
    ...
    if($ExcludeList -notcontains '5.7' -and $LevelOne){
        Service "(L1) Ensure Infrared monitor service (irmon) is set to Disabled or Not Installed" {
            Name = 'irmon'
            State = 'Stopped'
        }
    }
    ...
    if($ExcludeList -notcontains '17.1.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Credential Validation is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.2.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Application Group Management is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.2.2' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Security Group Management is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.2.3' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit User Account Management is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.3.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit PNP Activity is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.3.2' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Process Creation is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.5.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Account Lockout is set to include Failure (1)" {
            AuditFlag = 'Failure'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.5.2' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Group Membership is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.5.3' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Logoff is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
            Name = 'Logoff'
        }
    ...
    if($ExcludeList -notcontains '17.5.4' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Logon is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
            Name = 'Logon'
        }
    ...
    if($ExcludeList -notcontains '17.5.5' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Other LogonLogoff Events is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.5.6' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Special Logon is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
            Name = 'Special Logon'
        }
    ...
    if($ExcludeList -notcontains '17.6.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Detailed File Share is set to include Failure (1)" {
            AuditFlag = 'Failure'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.6.2' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit File Share is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
            Name = 'File Share'
        }
    ...
    if($ExcludeList -notcontains '17.6.3' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Other Object Access Events is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.6.4' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Removable Storage is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.7.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Audit Policy Change is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.7.2' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Authentication Policy Change is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.7.3' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Authorization Policy Change is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.7.4' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit MPSSVC RuleLevel Policy Change is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.7.5' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Other Policy Change Events is set to include Failure (1)" {
            AuditFlag = 'Failure'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.8.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Sensitive Privilege Use is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.9.1' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit IPsec Driver is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
            Name = 'IPsec Driver'
        }
    ...
    if($ExcludeList -notcontains '17.9.2' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Other System Events is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.9.3' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Security State Change is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.9.4' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit Security System Extension is set to include Success (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '17.9.5' -and $LevelOne){
        AuditPolicySubcategory "(L1) Ensure Audit System Integrity is set to Success and Failure (1)" {
            AuditFlag = 'Success'
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.3.5' -and $LevelOne){
        Registry "(L1) Ensure NetBT NodeType configuration is set to Enabled Pnode (recommended)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.5.9.1' -and $LevelTwo){
        Registry "(L2) Ensure Turn on Mapper IO (LLTDIO) driver is set to Disabled (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.5.9.2' -and $LevelTwo){
        Registry "(L2) Ensure Turn on Responder (RSPNDR) driver is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\LLTD'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.5.14.1' -and $LevelOne){
        Registry "(L1) Ensure Hardened UNC Paths is set to Enabled with Require Mutual Authentication and Require Integrity set for all NETLOGON and SYSVOL shares (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.5.20.1' -and $LevelTwo){
        Registry "(L2) Ensure Configuration of wireless settings using Windows Connect Now is set to Disabled (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.5.21.1' -and $LevelOne){
        Registry "(L1) Ensure Minimize the number of simultaneous connections to the Internet or a Windows Domain is set to Enabled 3  Prevent WiFi when on Ethernet" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.7.1.4' -and $BitLocker){
        Registry "(BL) Ensure Prevent installation of devices using drivers that match these device setup classes is set to Enabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\DeviceInstall\Restrictions'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.8.22.1.8' -and $LevelTwo){
        Registry "(L2) Ensure Turn off Registration if URL connection is referring to Microsoftcom is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.22.1.14' -and $LevelTwo){
        Registry "(L2) Ensure Turn off Windows Error Reporting is set to Enabled (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.25.1' -and $LevelTwo){
        Registry "(L2) Ensure Support device authentication using certificate is set to Enabled Automatic (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System\Kerberos\Parameters'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.8.36.1' -and $LevelOne){
        Registry "(L1) Ensure Configure Offer Remote Assistance is set to Disabled (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.36.2' -and $LevelOne){
        Registry "(L1) Ensure Configure Solicited Remote Assistance is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.5.1' -and $LevelOne){
        Registry "(L1) Ensure Let Windows apps activate with voice while the system is locked is set to Enabled Force Deny" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.11.3.1' -and $BitLocker){
        Registry "(BL) Ensure Allow access to BitLockerprotected removable data drives from earlier versions of Windows is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.11.1.14' -and $BitLocker){
        Registry "(BL) Ensure Configure use of passwords for fixed data drives is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.11.2.15' -and $BitLocker){
        Registry "(BL) Ensure Configure use of passwords for operating system drives is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.11.2.16' -and $BitLocker){
        Registry "(BL) Ensure Require additional authentication at startup is set to Enabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.11.3.14' -and $BitLocker){
        Registry "(BL) Ensure Configure use of passwords for removable data drives is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.15.3' -and $LevelOne){
        Registry "(L1) Ensure Prevent the use of security questions for local accounts is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.1' -and $LevelOne){
        Registry "(L1) Ensure Download Mode is NOT set to Enabled Internet" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.77.13.1.2' -and $LevelOne){
        Registry "(L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.78.6' -and $NextGenerationWindowsSecurity){
        Registry "(NG) Ensure Configure Windows Defender Application Guard clipboard settings Clipboard behavior setting is set to Enabled Enable clipboard operation from an isolated session to the host (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.80.1.1' -and $LevelOne){
        Registry "(L1) Ensure Configure Windows Defender SmartScreen is set to Enabled Warn and prevent bypass (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\System'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.86.1' -and $LevelOne){
        Registry "(L1) Ensure Signin and lock last interactive user automatically after a restart is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.95.1' -and $LevelOne){
        Registry "(L1) Ensure Turn on PowerShell Script Block Logging is set to Disabled (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.95.2' -and $LevelOne){
        Registry "(L1) Ensure Turn on PowerShell Transcription is set to Disabled (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.97.2.2' -and $LevelTwo){
        Registry "(L2) Ensure Allow remote server management through WinRM is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WinRM\Service'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.102.2' -and $LevelOne){
        Registry "(L1) Ensure Configure Automatic Updates is set to Enabled (1)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.102.1.1' -and $LevelOne){
        Registry "(L1) Ensure Manage preview builds is set to Enabled Disable preview builds (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.102.1.2' -and $LevelOne){
        Registry "(L1) Ensure Select when Preview Builds and Feature Updates are received is set to Enabled SemiAnnual Channel 180 or more days (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.102.1.3' -and $LevelOne){
        Registry "(L1) Ensure Select when Quality Updates are received is set to Enabled 0 days (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 1
