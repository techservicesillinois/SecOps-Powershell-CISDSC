Configuration CIS_Microsoft_Windows_10_Enterprise_Release_1809
{
    ...
    if($ExcludeList -notcontains '18.3.5' -and $LevelOne){
        Registry "18.3.5 - (L1) Ensure WDigest Authentication is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.5.4.1' -and $LevelOne){
        Registry "18.5.4.1 - (L1) Set NetBIOS node type to P-node (Ensure NetBT Parameter NodeType is set to 0x2 (2))" {
            Ensure = 'Present'
            Key = 'HKLM:\SYSTEM\CurrentControlSet\Services\Netbt\Parameters'
            ValueData = 2
            ValueName = 'NodeType'
    ...
    if($ExcludeList -notcontains '18.5.21.1' -and $LevelOne){
        Registry "18.5.21.1 - (L1) Ensure Minimize the number of simultaneous connections to the Internet or a Windows Domain is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.45.5.1' -and $LevelTwo){
        Registry "18.8.45.5.1 - (L2) Ensure Microsoft Support Diagnostic Tool Turn on MSDT interactive communication with support provider is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.45.11.1' -and $LevelTwo){
        Registry "18.8.45.11.1 - (L2) Ensure EnableDisable PerfTrack is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.47.1' -and $LevelTwo){
        Registry "18.8.47.1 - (L2) Ensure Turn off the advertising ID is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.50.1.1' -and $LevelTwo){
        Registry "18.8.50.1.1 - (L2) Ensure Enable Windows NTP Client is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.50.1.2' -and $LevelTwo){
        Registry "18.8.50.1.2 - (L2) Ensure Enable Windows NTP Server is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.11.4' -and $BitLocker){
        Registry "18.9.11.4 - (BL) Ensure Choose drive encryption method and cipher strength (Windows 10 Version 1511 and later) is set to Enabled XTS-AES 256-bit (1)" {
            Ensure = 'Present'
        ...
        Registry "18.9.11.4 - (BL) Ensure Choose drive encryption method and cipher strength (Windows 10 Version 1511 and later) is set to Enabled XTS-AES 256-bit (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 7
        ...
        Registry "18.9.11.4 - (BL) Ensure Choose drive encryption method and cipher strength (Windows 10 Version 1511 and later) is set to Enabled XTS-AES 256-bit (3)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 4
    ...
    if($ExcludeList -notcontains '18.9.11.5' -and $BitLocker){
        Registry "18.9.11.5 - (BL) Ensure Disable new DMA devices when this computer is locked is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\FVE'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.1' -and $LevelOne){
        Registry "18.9.17.1 - (L1) Ensure Download Mode is NOT set to Enabled Internet (3)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.39.2' -and $LevelTwo){
        Registry "18.9.39.2 - (L2) Ensure Turn off location is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.1' -and $LevelTwo){
        Registry "18.9.45.1 - (L2) Ensure Allow Address bar drop-down list suggestions is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.2' -and $LevelTwo){
        Registry "18.9.45.2 - (L2) Ensure Allow Adobe Flash is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.3' -and $LevelTwo){
        Registry "18.9.45.3 - (L2) Ensure Allow InPrivate Browsing is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.4' -and $LevelOne){
        Registry "18.9.45.4 - (L1) Ensure Allow Sideloading of extension is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.5' -and $LevelOne){
        Registry "18.9.45.5 - (L1) Ensure Configure cookies is set to Enabled Block only 3rd-party cookies or higher" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\MicrosoftEdge\Main'
            ValueData = 1
            ValueName = 'Cookies'
    ...
    if($ExcludeList -notcontains '18.9.45.6' -and $LevelOne){
        Registry "18.9.45.6 - (L1) Ensure Configure Password Manager is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\MicrosoftEdge\Main'
            ValueData = 'no'
    ...
    if($ExcludeList -notcontains '18.9.45.7' -and $LevelTwo){
        Registry "18.9.45.7 - (L2) Ensure Configure Pop-up Blocker is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\MicrosoftEdge\Main'
            ValueData = 'yes'
    ...
    if($ExcludeList -notcontains '18.9.45.8' -and $LevelTwo){
        Registry "18.9.45.8 - (L2) Ensure Configure search suggestions in Address bar is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.9' -and $LevelOne){
        Registry "18.9.45.9 - (L1) Ensure Configure the Adobe Flash Click-to-Run setting is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.10' -and $LevelTwo){
        Registry "18.9.45.10 - (L2) Ensure Prevent access to the aboutflags page in Microsoft Edge is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\MicrosoftEdge\Main'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.45.11' -and $LevelOne){
        Registry "18.9.45.11 - (L1) Ensure Prevent certificate error overrides is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.12' -and $LevelTwo){
        Registry "18.9.45.12 - (L2) Ensure Prevent using Localhost IP address for WebRTC is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\MicrosoftEdge\Main'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.58.1' -and $LevelTwo){
        Registry "18.9.58.1 - (L2) Ensure Turn off Push To Install service is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.59.3.2.1' -and $LevelTwo){
        Registry "18.9.59.3.2.1 - (L2) Ensure Allow users to connect remotely by using Remote Desktop Services is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.61.3' -and $LevelOne){
        Registry "18.9.61.3 - (L1) Ensure Allow Cortana is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Windows Search'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.61.4' -and $LevelOne){
        Registry "18.9.61.4 - (L1) Ensure Allow Cortana above lock screen is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Windows Search'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.61.5' -and $LevelOne){
        Registry "18.9.61.5 - (L1) Ensure Allow indexing of encrypted files is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Windows Search'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.61.6' -and $LevelOne){
        Registry "18.9.61.6 - (L1) Ensure Allow search and Cortana to use location is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Windows Search'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.69.1' -and $LevelTwo){
        Registry "18.9.69.1 - (L2) Ensure Disable all apps from Microsoft Store is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.69.2' -and $LevelOne){
        Registry "18.9.69.2 - (L1) Ensure Only display the private store within the Microsoft Store is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\WindowsStore'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.69.3' -and $LevelOne){
        Registry "18.9.69.3 - (L1) Ensure Turn off Automatic Download and Install of updates is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\WindowsStore'
            ValueData = 4
    ...
    if($ExcludeList -notcontains '18.9.69.4' -and $LevelOne){
        Registry "18.9.69.4 - (L1) Ensure Turn off the offer to update to the latest version of Windows is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\WindowsStore'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.69.5' -and $LevelTwo){
        Registry "18.9.69.5 - (L2) Ensure Turn off the Store application is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\WindowsStore'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.77.13.1.1' -and $LevelOne){
        Registry "18.9.77.13.1.1 - (L1) Ensure Configure Attack Surface Reduction rules is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.77.13.1.2' -and $LevelOne){
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (1)" {
            Ensure = 'Present'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (3)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (4)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (5)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (6)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (7)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (8)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (9)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (10)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.77.13.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (11)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
    ...
    if($ExcludeList -notcontains '18.9.78.1' -and $NextGenerationWindowsSecurity){
        Registry "18.9.78.1 - (NG) Ensure Allow auditing events in Windows Defender Application Guard is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.78.2' -and $NextGenerationWindowsSecurity){
        Registry "18.9.78.2 - (NG) Ensure Allow camera and microphone access in Windows Defender Application Guard is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.78.3' -and $NextGenerationWindowsSecurity){
        Registry "18.9.78.3 - (NG) Ensure Allow data persistence for Windows Defender Application Guard is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.78.4' -and $NextGenerationWindowsSecurity){
        Registry "18.9.78.4 - (NG) Ensure Allow files to download and save to the host operating system from Windows Defender Application Guard is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.78.5' -and $NextGenerationWindowsSecurity){
        Registry "18.9.78.5 - (NG) Ensure Allow users to trust files that open in Windows Defender Application Guard is set to Enabled 0 (Do not allow users to manually trust files) OR 2 (Allow users to manually trust after an antivirus check)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.78.6' -and $NextGenerationWindowsSecurity){
        Registry "18.9.78.6 - (NG) Ensure Configure Windows Defender Application Guard clipboard settings Clipboard behavior setting is set to Enabled Enable clipboard operation from an isolated session to the host (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 1
        ...
        Registry "18.9.78.6 - (NG) Ensure Configure Windows Defender Application Guard clipboard settings Clipboard behavior setting is set to Enabled Enable clipboard operation from an isolated session to the host (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.78.7' -and $NextGenerationWindowsSecurity){
        Registry "18.9.78.7 - (NG) Ensure Turn on Windows Defender Application Guard in Enterprise Mode is set to Enabled 1" {
            Ensure = 'Present'
            Key = 'HKLM:\SOFTWARE\Policies\Microsoft\AppHVSI'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.80.2.2' -and $LevelOne){
        Registry "18.9.80.2.2 - (L1) Ensure Prevent bypassing Windows Defender SmartScreen prompts for files is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\MicrosoftEdge\PhishingFilter'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.80.2.3' -and $LevelOne){
        Registry "18.9.80.2.3 - (L1) Ensure Prevent bypassing Windows Defender SmartScreen prompts for sites is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\MicrosoftEdge\PhishingFilter'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.86.1' -and $LevelOne){
        Registry "18.9.86.1 - (L1) Ensure Sign-in last interactive user automatically after a system-initiated restart is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System'
            ValueData = 1
