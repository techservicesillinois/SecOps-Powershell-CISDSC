Configuration CIS_Microsoft_Windows_Server_2022_Member_Server_Release_21H2
{
    param
    (
        [String[]]$ExcludeList = @(),
        ...
        [Int32]$cis18910842DeferFeatureUpdatesPeriodInDays = 180,
        [ValidateRange(32768,2147483647)]
        [Int32]$cis1892712MaxSize = 32768,
        [ValidateRange(196608,2147483647)]
        [Int32]$cis1892722MaxSize = 196608,
        [ValidateRange(32768,2147483647)]
        [Int32]$cis1892732MaxSize = 32768,
        [ValidateRange(32768,2147483647)]
        [Int32]$cis1892742MaxSize = 32768,
        [ValidateRange(60000,900000)]
        [Int32]$cis189653101MaxIdleTime = 900000,
    ...
    if($ExcludeList -notcontains '2.2.32' -and $LevelOne){
        UserRightsAssignment "2.2.32 - (L1) Ensure Impersonate a client after authentication is set to Administrators LOCAL SERVICE NETWORK SERVICE SERVICE and (when the Web Server (IIS) Role wit" {
            Force = $true
            Identity = @('*S-1-5-19','*S-1-5-20','*S-1-5-32-544','*S-1-5-6')
    ...
    if($ExcludeList -notcontains '2.2.33' -and $LevelOne){
        UserRightsAssignment "2.2.33 - (L1) Ensure Increase scheduling priority is set to Administrators Window ManagerWindow Manager Group" {
            Force = $true
    ...
    if($ExcludeList -notcontains '2.3.10.8' -and $LevelOne){
        Registry "2.3.10.8 - (L1) Configure Network access Remotely accessible registry paths is configured" {
    ...
    if($ExcludeList -notcontains '2.3.10.9' -and $LevelOne){
        Registry "2.3.10.9 - (L1) Configure Network access Remotely accessible registry paths and sub-paths is configured" {
    ...
    if($ExcludeList -notcontains '5.2' -and $LevelTwo){
        CISService "5.2 - (L2) Ensure Print Spooler (Spooler) is set to Disabled (MS only)" {
            Name = 'Spooler'
        }
    }
    ...
    if($ExcludeList -notcontains '18.3.5' -and $LevelOne){
        Registry "18.3.5 - (L1) Ensure Limits print driver installation to Administrators is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.4.2' -and $LevelOne){
        Registry "18.4.2 - (L1) Ensure MSS (DisableIPSourceRouting IPv6) IP source routing protection level (protects against packet spoofing) is set to Enabled Highest protection sou" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.4.3' -and $LevelOne){
        Registry "18.4.3 - (L1) Ensure MSS (DisableIPSourceRouting) IP source routing protection level (protects against packet spoofing) is set to Enabled Highest protection source r" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.4.9' -and $LevelOne){
        Registry "18.4.9 - (L1) Ensure MSS (ScreenSaverGracePeriod) The time in seconds before the screen saver grace period expires (0 recommended) is set to Enabled 5 or fewer secon" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Microsoft\Windows NT\CurrentVersion\Winlogon'
    ...
    if($ExcludeList -notcontains '18.5.4.1' -and $LevelOne){
        Registry "18.5.4.1 - (L1) Ensure Configure DNS over HTTPS (DoH) name resolution is set to Enabled Allow DoH or higher" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.5.4.2' -and $LevelOne){
        Registry "18.5.4.2 - (L1) Ensure Turn off multicast name resolution is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\DNSClient'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.6.1' -and $LevelOne){
        Registry "18.6.1 - (L1) Ensure Allow Print Spooler to accept client connections is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.6.2' -and $LevelOne){
        Registry "18.6.2 - (L1) Ensure Point and Print Restrictions When installing drivers for a new connection is set to Enabled Show warning and elevation prompt" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Printers\PointAndPrint'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.6.3' -and $LevelOne){
        Registry "18.6.3 - (L1) Ensure Point and Print Restrictions When updating drivers for an existing connection is set to Enabled Show warning and elevation prompt" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Printers\PointAndPrint'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.8.3.1' -and $LevelOne){
        Registry "18.8.3.1 - (L1) Ensure Include command line in process creation events is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.7.2' -and $LevelOne){
        Registry "18.8.7.2 - (L1) Ensure Prevent device metadata retrieval from the Internet is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.26.1' -and $LevelOne){
        Registry "18.8.26.1 - (L1) Ensure Enumeration policy for external devices incompatible with Kernel DMA Protection is set to Enabled Block All" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.48.5.1' -and $LevelTwo){
        Registry "18.8.48.5.1 - (L2) Ensure Microsoft Support Diagnostic Tool Turn on MSDT interactive communication with support provider is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.48.11.1' -and $LevelTwo){
        Registry "18.8.48.11.1 - (L2) Ensure EnableDisable PerfTrack is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.50.1' -and $LevelTwo){
        Registry "18.8.50.1 - (L2) Ensure Turn off the advertising ID is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.53.1.1' -and $LevelTwo){
        Registry "18.8.53.1.1 - (L2) Ensure Enable Windows NTP Client is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.8.53.1.2' -and $LevelTwo){
        Registry "18.8.53.1.2 - (L2) Ensure Enable Windows NTP Server is set to Disabled (MS only)" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.14.1' -and $LevelOne){
        Registry "18.9.14.1 - (L1) Ensure Turn off cloud consumer account state content is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.14.2' -and $LevelOne){
        Registry "18.9.14.2 - (L1) Ensure Turn off Microsoft consumer experiences is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\CloudContent'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.15.1' -and $LevelOne){
        Registry "18.9.15.1 - (L1) Ensure Require pin for pairing is set to Enabled First Time OR Enabled Always" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.16.1' -and $LevelOne){
        Registry "18.9.16.1 - (L1) Ensure Do not display the password reveal button is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.16.2' -and $LevelOne){
        Registry "18.9.16.2 - (L1) Ensure Enumerate administrator accounts on elevation is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.17.1' -and $LevelOne){
        Registry "18.9.17.1 - (L1) Ensure Allow Diagnostic Data is set to Enabled Diagnostic data off (not recommended) or Enabled Send required diagnostic data" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.17.2' -and $LevelTwo){
        Registry "18.9.17.2 - (L2) Ensure Configure Authenticated Proxy usage for the Connected User Experience and Telemetry service is set to Enabled Disable Authenticated Proxy usa" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\DataCollection'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.3' -and $LevelOne){
        Registry "18.9.17.3 - (L1) Ensure Disable OneSettings Downloads is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\DataCollection'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.4' -and $LevelOne){
        Registry "18.9.17.4 - (L1) Ensure Do not show feedback notifications is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\DataCollection'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.5' -and $LevelOne){
        Registry "18.9.17.5 - (L1) Ensure Enable OneSettings Auditing is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\DataCollection'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.6' -and $LevelOne){
        Registry "18.9.17.6 - (L1) Ensure Limit Diagnostic Log Collection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\DataCollection'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.7' -and $LevelOne){
        Registry "18.9.17.7 - (L1) Ensure Limit Dump Collection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\DataCollection'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.17.8' -and $LevelOne){
        Registry "18.9.17.8 - (L1) Ensure Toggle user control over Insider builds is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.27.1.1' -and $LevelOne){
        Registry "18.9.27.1.1 - (L1) Ensure Application Control Event Log behavior when the log file reaches its maximum size is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.27.1.2' -and $LevelOne){
        Registry "18.9.27.1.2 - (L1) Ensure Application Specify the maximum log file size (KB) is set to Enabled 32768 or greater" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\EventLog\Application'
            ValueData = $cis1892712MaxSize
            ValueName = 'MaxSize'
            ValueType = 'Dword'
        }
    }
    if($ExcludeList -notcontains '18.9.27.2.1' -and $LevelOne){
        Registry "18.9.27.2.1 - (L1) Ensure Security Control Event Log behavior when the log file reaches its maximum size is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.27.2.2' -and $LevelOne){
        Registry "18.9.27.2.2 - (L1) Ensure Security Specify the maximum log file size (KB) is set to Enabled 196608 or greater" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\EventLog\Security'
            ValueData = $cis1892722MaxSize
            ValueName = 'MaxSize'
            ValueType = 'Dword'
        }
    }
    if($ExcludeList -notcontains '18.9.27.3.1' -and $LevelOne){
        Registry "18.9.27.3.1 - (L1) Ensure Setup Control Event Log behavior when the log file reaches its maximum size is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.27.3.2' -and $LevelOne){
        Registry "18.9.27.3.2 - (L1) Ensure Setup Specify the maximum log file size (KB) is set to Enabled 32768 or greater" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\EventLog\Setup'
            ValueData = $cis1892732MaxSize
            ValueName = 'MaxSize'
            ValueType = 'Dword'
        }
    }
    if($ExcludeList -notcontains '18.9.27.4.1' -and $LevelOne){
        Registry "18.9.27.4.1 - (L1) Ensure System Control Event Log behavior when the log file reaches its maximum size is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.27.4.2' -and $LevelOne){
        Registry "18.9.27.4.2 - (L1) Ensure System Specify the maximum log file size (KB) is set to Enabled 32768 or greater" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\EventLog\System'
            ValueData = $cis1892742MaxSize
            ValueName = 'MaxSize'
            ValueType = 'Dword'
        }
    }
    if($ExcludeList -notcontains '18.9.31.2' -and $LevelOne){
        Registry "18.9.31.2 - (L1) Ensure Turn off Data Execution Prevention for Explorer is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Explorer'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.31.3' -and $LevelOne){
        Registry "18.9.31.3 - (L1) Ensure Turn off heap termination on corruption is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Explorer'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.31.4' -and $LevelOne){
        Registry "18.9.31.4 - (L1) Ensure Turn off shell protocol protected mode is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.41.1' -and $LevelTwo){
        Registry "18.9.41.1 - (L2) Ensure Turn off location is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.45.1' -and $LevelTwo){
        Registry "18.9.45.1 - (L2) Ensure Allow Message Service Cloud Sync is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.46.1' -and $LevelOne){
        Registry "18.9.46.1 - (L1) Ensure Block all consumer Microsoft account user authentication is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.15' -and $LevelOne){
        Registry "18.9.47.15 - (L1) Ensure Configure detection for potentially unwanted applications is set to Enabled Block" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.16' -and $LevelOne){
        Registry "18.9.47.16 - (L1) Ensure Turn off Microsoft Defender AntiVirus is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.47.4.1' -and $LevelOne){
        Registry "18.9.47.4.1 - (L1) Ensure Configure local setting override for reporting to Microsoft MAPS is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.4.2' -and $LevelTwo){
        Registry "18.9.47.4.2 - (L2) Ensure Join Microsoft MAPS is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Spynet'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.47.6.1' -and $LevelTwo){
        Registry "18.9.47.6.1 - (L2) Ensure Enable file hash computation feature is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.9.1' -and $LevelOne){
        Registry "18.9.47.9.1 - (L1) Ensure Scan all downloaded files and attachments is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.9.2' -and $LevelOne){
        Registry "18.9.47.9.2 - (L1) Ensure Turn off real-time protection is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Real-Time Protection'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.47.9.3' -and $LevelOne){
        Registry "18.9.47.9.3 - (L1) Ensure Turn on behavior monitoring is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Real-Time Protection'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.47.9.4' -and $LevelOne){
        Registry "18.9.47.9.4 - (L1) Ensure Turn on script scanning is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Real-Time Protection'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.47.11.1' -and $LevelTwo){
        Registry "18.9.47.11.1 - (L2) Ensure Configure Watson events is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.12.1' -and $LevelOne){
        Registry "18.9.47.12.1 - (L1) Ensure Scan removable drives is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.12.2' -and $LevelOne){
        Registry "18.9.47.12.2 - (L1) Ensure Turn on e-mail scanning is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Scan'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.47.5.1.1' -and $LevelOne){
        Registry "18.9.47.5.1.1 - (L1) Ensure Configure Attack Surface Reduction rules is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.47.5.1.2' -and $LevelOne){
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (1)" {
            Ensure = 'Present'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (3)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (4)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (5)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (6)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (7)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (8)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (9)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (10)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (11)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
        ...
        Registry "18.9.47.5.1.2 - (L1) Ensure Configure Attack Surface Reduction rules Set the state for each ASR rule is configured (12)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows Defender\Windows Defender Exploit Guard\ASR\Rules'
            ValueData = '1'
    ...
    if($ExcludeList -notcontains '18.9.47.5.3.1' -and $LevelOne){
        Registry "18.9.47.5.3.1 - (L1) Ensure Prevent users and apps from accessing dangerous websites is set to Enabled Block" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.58.1' -and $LevelOne){
        Registry "18.9.58.1 - (L1) Ensure Prevent the usage of OneDrive for file storage is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.64.1' -and $LevelTwo){
        Registry "18.9.64.1 - (L2) Ensure Turn off Push To Install service is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.65.2.2' -and $LevelOne){
        Registry "18.9.65.2.2 - (L1) Ensure Do not allow passwords to be saved is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.2.1' -and $LevelTwo){
        Registry "18.9.65.3.2.1 - (L2) Ensure Restrict Remote Desktop Services users to a single Remote Desktop Services session is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.3.1' -and $LevelTwo){
        Registry "18.9.65.3.3.1 - (L2) Ensure Allow UI Automation redirection is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.65.3.3.2' -and $LevelTwo){
        Registry "18.9.65.3.3.2 - (L2) Ensure Do not allow COM port redirection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.3.3' -and $LevelOne){
        Registry "18.9.65.3.3.3 - (L1) Ensure Do not allow drive redirection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.3.4' -and $LevelTwo){
        Registry "18.9.65.3.3.4 - (L2) Ensure Do not allow location redirection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.3.5' -and $LevelTwo){
        Registry "18.9.65.3.3.5 - (L2) Ensure Do not allow LPT port redirection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.3.6' -and $LevelTwo){
        Registry "18.9.65.3.3.6 - (L2) Ensure Do not allow supported Plug and Play device redirection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.9.1' -and $LevelOne){
        Registry "18.9.65.3.9.1 - (L1) Ensure Always prompt for password upon connection is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.9.2' -and $LevelOne){
        Registry "18.9.65.3.9.2 - (L1) Ensure Require secure RPC communication is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.9.3' -and $LevelOne){
        Registry "18.9.65.3.9.3 - (L1) Ensure Require use of specific security layer for remote (RDP) connections is set to Enabled SSL" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 2
    ...
    if($ExcludeList -notcontains '18.9.65.3.9.4' -and $LevelOne){
        Registry "18.9.65.3.9.4 - (L1) Ensure Require user authentication for remote connections by using Network Level Authentication is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.9.5' -and $LevelOne){
        Registry "18.9.65.3.9.5 - (L1) Ensure Set client connection encryption level is set to Enabled High Level" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 3
    ...
    if($ExcludeList -notcontains '18.9.65.3.10.1' -and $LevelTwo){
        Registry "18.9.65.3.10.1 - (L2) Ensure Set time limit for active but idle Remote Desktop Services sessions is set to Enabled 15 minutes or less but not Never (0)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = $cis189653101MaxIdleTime
            ValueName = 'MaxIdleTime'
            ValueType = 'Dword'
        }
    }
    if($ExcludeList -notcontains '18.9.65.3.10.2' -and $LevelTwo){
        Registry "18.9.65.3.10.2 - (L2) Ensure Set time limit for disconnected sessions is set to Enabled 1 minute" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 60000
    ...
    if($ExcludeList -notcontains '18.9.65.3.11.1' -and $LevelOne){
        Registry "18.9.65.3.11.1 - (L1) Ensure Do not delete temp folders upon exit is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.65.3.11.2' -and $LevelOne){
        Registry "18.9.65.3.11.2 - (L1) Ensure Do not use temporary folders per session is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.66.1' -and $LevelOne){
        Registry "18.9.66.1 - (L1) Ensure Prevent downloading of enclosures is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.67.2' -and $LevelTwo){
        Registry "18.9.67.2 - (L2) Ensure Allow Cloud Search is set to Enabled Disable Cloud Search" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.67.3' -and $LevelOne){
        Registry "18.9.67.3 - (L1) Ensure Allow indexing of encrypted files is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Windows Search'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.72.1' -and $LevelTwo){
        Registry "18.9.72.1 - (L2) Ensure Turn off KMS Client Online AVS Validation is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.85.1.1' -and $LevelOne){
        Registry "18.9.85.1.1 - (L1) Ensure Configure Windows Defender SmartScreen is set to Enabled Warn and prevent bypass (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\System'
            ValueData = 1
        ...
        Registry "18.9.85.1.1 - (L1) Ensure Configure Windows Defender SmartScreen is set to Enabled Warn and prevent bypass (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\System'
            ValueData = 'Block'
    ...
    if($ExcludeList -notcontains '18.9.89.1' -and $LevelTwo){
        Registry "18.9.89.1 - (L2) Ensure Allow suggested apps in Windows Ink Workspace is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.89.2' -and $LevelOne){
        Registry "18.9.89.2 - (L1) Ensure Allow Windows Ink Workspace is set to Enabled On but disallow access above lock OR Disabled but not Enabled On" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\WindowsInkWorkspace'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.90.1' -and $LevelOne){
        Registry "18.9.90.1 - (L1) Ensure Allow user control over installs is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.90.2' -and $LevelOne){
        Registry "18.9.90.2 - (L1) Ensure Always install with elevated privileges is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Installer'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.90.3' -and $LevelTwo){
        Registry "18.9.90.3 - (L2) Ensure Prevent Internet Explorer security prompt for Windows Installer scripts is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Installer'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.91.1' -and $LevelOne){
        Registry "18.9.91.1 - (L1) Ensure Sign-in and lock last interactive user automatically after a restart is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Policies\System'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.100.1' -and $LevelOne){
        Registry "18.9.100.1 - (L1) Ensure Turn on PowerShell Script Block Logging is set to Enabled (1)" {
            Ensure = 'Present'
        ...
        Registry "18.9.100.1 - (L1) Ensure Turn on PowerShell Script Block Logging is set to Enabled (2)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging'
            ValueName = 'EnableScriptBlockInvocationLogging'
        }
    }
    if($ExcludeList -notcontains '18.9.100.2' -and $LevelOne){
        Registry "18.9.100.2 - (L1) Ensure Turn on PowerShell Transcription is set to Disabled (1)" {
            Ensure = 'Present'
        ...
        Registry "18.9.100.2 - (L1) Ensure Turn on PowerShell Transcription is set to Disabled (2)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\PowerShell\Transcription'
            ValueName = 'OutputDirectory'
        }
        Registry "18.9.100.2 - (L1) Ensure Turn on PowerShell Transcription is set to Disabled (3)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\PowerShell\Transcription'
    ...
    if($ExcludeList -notcontains '18.9.102.1.1' -and $LevelOne){
        Registry "18.9.102.1.1 - (L1) Ensure Allow Basic authentication is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.102.1.2' -and $LevelOne){
        Registry "18.9.102.1.2 - (L1) Ensure Allow unencrypted traffic is set to Disabled (1)" {
            Ensure = 'Present'
        ...
        Registry "18.9.102.1.2 - (L1) Ensure Allow unencrypted traffic is set to Disabled (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WinRM\Client'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.102.1.3' -and $LevelOne){
        Registry "18.9.102.1.3 - (L1) Ensure Disallow Digest authentication is set to Enabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 0
        ...
        Registry "18.9.102.1.3 - (L1) Ensure Disallow Digest authentication is set to Enabled (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WinRM\Client'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.102.2.1' -and $LevelOne){
        Registry "18.9.102.2.1 - (L1) Ensure Allow Basic authentication is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.102.2.2' -and $LevelTwo){
        Registry "18.9.102.2.2 - (L2) Ensure Allow remote server management through WinRM is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WinRM\Service'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.102.2.3' -and $LevelOne){
        Registry "18.9.102.2.3 - (L1) Ensure Allow unencrypted traffic is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WinRM\Service'
            ValueData = 0
            ValueName = 'AllowUnencryptedTraffic'
    ...
    if($ExcludeList -notcontains '18.9.102.2.4' -and $LevelOne){
        Registry "18.9.102.2.4 - (L1) Ensure Disallow WinRM from storing RunAs credentials is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WinRM\Service'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.103.1' -and $LevelTwo){
        Registry "18.9.103.1 - (L2) Ensure Allow Remote Shell Access is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.105.2.1' -and $LevelOne){
        Registry "18.9.105.2.1 - (L1) Ensure Prevent users from modifying settings is set to Enabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.108.1.1' -and $LevelOne){
        Registry "18.9.108.1.1 - (L1) Ensure No auto-restart with logged on users for scheduled automatic updates installations is set to Disabled" {
            Ensure = 'Present'
    ...
    if($ExcludeList -notcontains '18.9.108.2.1' -and $LevelOne){
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueData = 0
        ...
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueData = 3
            ValueName = 'AUOptions'
        ...
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (3)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueName = 'AutomaticMaintenanceEnabled'
        }
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (4)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueData = 3
        ...
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (5)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueData = 1
        ...
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (6)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueName = 'ScheduledInstallFirstWeek'
        }
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (7)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueName = 'ScheduledInstallSecondWeek'
        }
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (8)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueName = 'ScheduledInstallThirdWeek'
        }
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (9)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueName = 'ScheduledInstallFourthWeek'
        }
        Registry "18.9.108.2.1 - (L1) Ensure Configure Automatic Updates is set to Enabled (10)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueName = 'AllowMUUpdateService'
        }
    }
    if($ExcludeList -notcontains '18.9.108.2.2' -and $LevelOne){
        Registry "18.9.108.2.2 - (L1) Ensure Configure Automatic Updates Scheduled install day is set to 0 - Every day" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU'
            ValueData = 0
    ...
    if($ExcludeList -notcontains '18.9.108.4.1' -and $LevelOne){
        Registry "18.9.108.4.1 - (L1) Ensure Manage preview builds is set to Disabled (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 1
        ...
        Registry "18.9.108.4.1 - (L1) Ensure Manage preview builds is set to Disabled (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 1
        ...
        Registry "18.9.108.4.1 - (L1) Ensure Manage preview builds is set to Disabled (3)" {
            Ensure = 'Absent'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate'
            ValueName = 'BranchReadinessLevel'
        }
    }
    if($ExcludeList -notcontains '18.9.108.4.2' -and $LevelOne){
        Registry "18.9.108.4.2 - (L1) Ensure Select when Preview Builds and Feature Updates are received is set to Enabled 180 or more days (1)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 1
        ...
        Registry "18.9.108.4.2 - (L1) Ensure Select when Preview Builds and Feature Updates are received is set to Enabled 180 or more days (2)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = $cis18910842DeferFeatureUpdatesPeriodInDays
            ValueName = 'DeferFeatureUpdatesPeriodInDays'
            ValueType = 'Dword'
        }
    }
    if($ExcludeList -notcontains '18.9.108.4.3' -and $LevelOne){
        ...
        Registry "18.9.108.4.3 - (L1) Ensure Select when Quality Updates are received is set to Enabled 0 days (32)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate'
            ValueData = 0
