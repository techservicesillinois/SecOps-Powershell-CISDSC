Configuration CIS_Microsoft_Windows_Server_2019_Member_Server_Release_20H2
{
    param
    (
        [String[]]$ExcludeList = @(),
    ...
    if($ExcludeList -notcontains '18.9.62.3.2.1' -and $LevelTwo){
        Registry "18.9.62.3.2.1 - (L2) Ensure Restrict Remote Desktop Services users to a single Remote Desktop Services session is set to Enabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.62.3.10.1' -and $LevelTwo){
        Registry "18.9.62.3.10.1 - (L2) Ensure Set time limit for active but idle Remote Desktop Services sessions is set to Enabled 15 minutes or less but not Never (0)" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
    ...
    if($ExcludeList -notcontains '18.9.62.3.11.2' -and $LevelOne){
        Registry "18.9.62.3.11.2 - (L1) Ensure Do not use temporary folders per session is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows NT\Terminal Services'
            ValueData = 1
    ...
    if($ExcludeList -notcontains '18.9.64.3' -and $LevelOne){
        Registry "18.9.64.3 - (L1) Ensure Allow indexing of encrypted files is set to Disabled" {
            Ensure = 'Present'
            Key = 'HKLM:\Software\Policies\Microsoft\Windows\Windows Search'
            ValueData = 0
